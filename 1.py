...
client = LINE()
#client = LINE("EtOg7Kblq5bICyTb2sOd.1/gYgy9dVI5WSGdulQZ83q.l6HbP88yO7Sq/5Lw80ScObwO15NwCIP8TF/xFb9LgcU=")
#client = LINE("stepharu2326@gmail.com","renaldi2326")
                    ...
                    if msg.contentType == 0:
                        if text is None:
                            ...
                            if cmd == "help":
                                ...
                                client.sendMessageWithContent(to, text, name, link, icon)
                            elif cmd == "tts":
...
# Pembatas Script #
                                ...
                                sendMention(to, "@!", [sender])
                                client.sendMessageWithContent(to, text, name, link, icon)
                                client.sendMessage(to,"vlalavlavla")
                            elif cmd == "mymid":
                                ...
                                if msg._from in admin:    
                                 try:
                                     ...
                                     client.sendImageWithURL(msg.to, linkpic)
...
# Pembatas Script #   
                                ...
                                try:
                                    ...
                                    logError(error)
                            elif cmd.startswith("anistream"):
                            	 eins = msg.text.split("-")
                            	 r = requests.get("http://moeapi.xyz/api/anistream/newpost/?apikey=beta")
                            	 data = r.text
                            	 data = json.loads(data)
                            	 if len(cond) == 1:
                                 	num = 0
                                 	...
                                 	for anime in data["result"]:
                                 	    num += 1
                                 	    ret_ +="| {}.  {}".format(str(num), str(anime["title"]))
                                 	ret_ +="| [ total {} anime ] ".format(str(len(data["result"])))
                                    ...
                                    if num <= len(data["result"]):
                                      ...
                                      r = result.get("http://moeapi.xyz/api/anistream/dl/?apikey=beta&url{}".format (str(anime[url])))
                                      data = r.text
                                      data = json.loads(data)
                                      if data["result"] != []:
                                         ret_ = "| [ Anime ] "
                                         ret_ = "\n | Click the link if you want strean"
                                         ret_ = "\n | [ Finish ] "
                                         link = "{}" .format(str(data["result"]["thumbnail"]))
                                         urlstream = "{}" .format(str(data["result"]["url"]))
                                         client.sendImageWithURL(to, link)
                                         client.sendMessage(to, str(ret_))
                                         client.sendMessage(to, str (urlstream))
